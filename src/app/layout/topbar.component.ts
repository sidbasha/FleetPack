  ...
  template: `
    <header class="h-14 sticky top-0 z-20 bg-white/90 backdrop-blur-sm border-b border-slate-200 flex items-center gap-4 px-5">
      ...
      <div class="relative flex items-center gap-3 pl-3 border-l border-slate-200">
        <button class="text-slate-400 hover:text-indigo-600" title="Notifications">🔔</button>
        <button class="text-slate-400 hover:text-indigo-600" title="Messages">✉</button>
        <button
          type="button"
...
export class TopbarComponent implements OnInit {
  ...
  onFleet(e: Event): void { this.store.setFleet((e.target as HTMLSelectElement).value); }
  onDuration(e: Event): void { this.store.setDuration((e.target as HTMLSelectElement).value as GlobalFilters['duration']); }
}
